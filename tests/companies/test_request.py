...
def test_registration_rejects_invalid_linkedin_urls(url: str) -> None:
    with pytest.raises(ValidationError):
        CompanyRegistrationRequest.model_validate(_payload(url))


def test_invalid_linkedin_port_does_not_leak_the_parser_message() -> None:
    with pytest.raises(ValidationError) as error:
        CompanyRegistrationRequest.model_validate(
            _payload("https://linkedin.com:abc/company/test")
        )
    message = str(error.value)
    assert "use an HTTPS LinkedIn company URL" in message
    assert "cast to integer" not in message
